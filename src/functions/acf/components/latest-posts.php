...
function component_latest_posts_html( string $output, string $layout ): string {
    ...
    $items_count    = '2'; // Force to always show 2 items
    ...
    <section class="rfs-ref-latest-posts-section ats-latest-posts-section py-8 lg:py-12 <?php echo esc_attr( $bg_class ); ?>">
        <div class="rfs-ref-latest-posts-container container mx-auto px-4">
            ...
            <!-- Posts Grid - Always 2 items side by side -->
            <div class="rfs-ref-latest-posts-grid grid grid-cols-1 lg:grid-cols-2 gap-8">
                <?php foreach ( $items as $item ) :
                    ...
                    $thumbnail_url = $thumbnail_id ? wpimage( image: $thumbnail_id, size: [400, 300], retina: true, webp: true ) : '';
                ?>
                    <article class="rfs-ref-latest-post-card ats-latest-card flex flex-col md:flex-row gap-6 items-start">
                        <!-- Image on LEFT - 40% of card width -->
                        <?php if ( $thumbnail_url ) : ?>
                            <a href="<?php echo esc_url( $post_url ); ?>" class="rfs-ref-latest-post-image-link flex-shrink-0 w-full md:w-[40%]">
                                <img
                        ...
                        <?php else : ?>
                            <a href="<?php echo esc_url( $post_url ); ?>" class="rfs-ref-latest-post-placeholder flex-shrink-0 w-full md:w-[40%] bg-gray-100 flex items-center justify-center h-48 rounded-xl">
                        ...
                        <!-- Content on RIGHT -->
                        <div class="rfs-ref-latest-post-content flex flex-col flex-grow">
                            <h3 class="rfs-ref-latest-post-title text-lg lg:text-xl font-bold text-primary-700 mb-3 leading-snug">
                                <a href="<?php echo esc_url( $post_url ); ?>" class="rfs-ref-latest-post-title-link hover:text-primary-800">
                            ...
                            <?php if ( $post_excerpt ) : ?>
                                <p class="rfs-ref-latest-post-excerpt text-black text-sm leading-relaxed mb-4">
