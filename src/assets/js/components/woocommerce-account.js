	...
	if (avatarUpload && avatarDisplay) {
		avatarUpload.addEventListener('change', function (e) {
			...
			if (file && file.type.startsWith('image/')) {
				...
				// Get nonce from the data attribute we added in the form
				const dataContainer = document.getElementById('woocommerce-account-data');
				const nonce = dataContainer ? dataContainer.dataset.nonce : '';
				formData.append('nonce', nonce);

				// Use themeData.ajax_url if available, otherwise fallback
				const ajaxUrl = typeof themeData !== 'undefined' && themeData.ajax_url ? themeData.ajax_url : '/wp-admin/admin-ajax.php';
				...
				fetch(ajaxUrl, {
					...
					.then((data) => {
						if (data.success) {
							// Update avatar src with the returned URL
							if (data.data.url) {
								avatarDisplay.src = data.data.url;
								// Also update any other avatars on the page if needed
								const headerAvatar = document.querySelector('.header-avatar-img'); // Example selector
								if (headerAvatar) headerAvatar.src = data.data.url;
							}
