...
function heading_data( $selected = false ) {
    ...
    if ( $data['phone'] ) {
        $data['phone_html'] = '<div class="tel inline-flex items-center space-x-2 text-ats-text">
			<svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor" style="transform: scaleX(-1);"><path d="M235.77-185q-21.33 2-37.55-12.29Q182-211.57 182-233v-71.38q0-20.08 11.15-34.5 11.16-14.43 30.47-19.43l53.15-12.23q13.77-2 23.46.35 9.69 2.34 18.46 11.88l90.54 89.08q48-24.62 88.15-53.58 40.16-28.96 74.04-62.82 32.73-33.29 61.04-72.14 28.31-38.85 52.92-84.54l-87.76-81.84q-9.54-8-13.81-21.54-4.27-13.54-1.5-27.31l17.84-61.38q6.31-19.08 19.97-30.35Q633.77-776 653.08-776H726q21.43 0 35.71 15.72Q776-744.56 774-723.23q-10 97.15-55.65 189-45.66 91.85-122.81 169.38-77.16 77.54-169.89 124.2Q332.92-194 235.77-185Zm464.15-388.85Q717-615 728.69-655.12q11.69-40.11 12.85-78.11 0-4.62-3.08-7.69-3.08-3.08-7.69-3.08h-83.85q-6.15 0-10 3.08-3.84 3.07-5.38 9.23l-17 64.77q-1.54 4.61-.39 10 1.16 5.38 5 8.46l80.77 74.61Zm-324 320.93-79.23-80.77q-3.84-3.85-7.31-5-3.46-1.16-8.07.38l-55 11.39q-6.16 1.54-9.23 5.38-3.08 3.85-3.08 10v82.31q0 4.61 3.08 7.69 3.07 3.08 7.69 3.08 30.08-2.69 72.35-10.04 42.26-7.35 78.8-24.42Zm324-320.93Zm-324 320.93Z"/></svg>
    ...
    if ( $main_logo ) {
        ...
        if ($logo_id) {
            $img_1x = wpimage( image: $logo_id, size: 300, retina: false );
            $img_2x = wpimage( image: $logo_id, size: 300, retina: true );
    ...
    if ( $navigation_links && is_array( $navigation_links ) ) {
        ...
        foreach ( $navigation_links as $nav_item ) {
            ...
            if ( $link && isset( $link['url'], $link['title'] ) ) {
                ...
                $navigation_html .= '<li><a href="' . esc_url( $link['url'] ) . '" class="rfs-ref-header-nav-link text-ats-text text-xs font-normal hover:text-ats-yellow"' . $target_attr . '>' . esc_html( $link['title'] ) . '</a></li>';
            }
        }
        $navigation_html .= '</ul>';
    }
